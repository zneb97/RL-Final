...
if __name__ == "__main__":
    ...
    memory_0 = ReplayMemory(memory_size, batch_size)
    memory_1 = ReplayMemory(memory_size, batch_size/4)
    agent = Agent(strategy, em.num_actions_available(), device, memory_1)

    policy_net = DQN(em.num_actions_available(), device).to(device)
    policy_net.load_state_dict(torch.load("breakout-v0-1e.pth"))
    ...
    #         next_state = em.get_state()
    #         if reward.item() == 0:
    #             memory_0.push(Experience(state, action, next_state, reward))
    #         else:
    #             memory_1.push(Experience(state, action, next_state, reward))
    #         state = next_state

    #         loss = None
    #         if memory_0.can_provide_sample() and memory_1.can_provide_sample():
    #             experiences = memory_0.sample() + memory_1.sample()
    ...
    #             print(" Memory len:", memory_0.push_count + memory_1.push_count)
    #             break
